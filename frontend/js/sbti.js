...
const SBTI_TO_MBTI = {
  ...
  "CTRL": "INTP, INTJ, ENTJ",
  "HHHH": "ESFP - 表演者",
  "SEXY": "ENFP, ESFP, ENFJ, INFJ",
  "OJBK": "ESTJ - 总经理",
  "JOKE-R": "INFP, ENFP, INFJ",
  "POOR": "ISFP - 探险家",
  "OH-NO": "ISTJ - 物流师",
  "MONK": "ISFP, ENTP, INTJ",
  "SHIT": "ENTP, ESTJ",
...
function displayQuestion() {
  app.visibleQuestions = getVisibleQuestions();
  const q = app.visibleQuestions[app.currentQuestion];
  if (!q) return;

  questionText.textContent = `问题${app.currentQuestion + 1}：${q.text}`;

  const progress = ((app.currentQuestion + 1) / app.visibleQuestions.length) * 100;
  progressBar.style.width = `${progress}%`;
  progressText.textContent = `${app.currentQuestion + 1} / ${app.visibleQuestions.length}`;
  ...
  optionsContainer.innerHTML = q.options.map((opt) => {
    ...
    return `<div class="option ${isSelected ? 'selected' : ''}" onclick="window.__sbti_select(${Number(opt.value)})"><span class="option-text">${opt.label}</span></div>`;
  }).join('');

  prevBtn.style.display = app.currentQuestion > 0 ? 'inline-block' : 'none';
  const isLast = app.currentQuestion === app.visibleQuestions.length - 1;
...
function selectAnswer(value) {
  ...
  const isLastQuestion = app.currentQuestion === app.visibleQuestions.length - 1;
  if (!isLastQuestion) {
    goToNext();
  } else {
    updateProgress();
...
function goToNext() {
  ...
  const total = app.visibleQuestions.length;
...
async function handleSubmitTest() {
  const unansweredCount = app.visibleQuestions.filter(q => app.answers[q.id] === undefined).length;
...
function updateProgress() {
  const done = app.visibleQuestions.filter(q => app.answers[q.id] !== undefined).length;
  const total = app.visibleQuestions.length;
...
function renderResult() {
  ...
  const mbtiMatch = SBTI_TO_MBTI[type.code] || '未找到对应 MBTI';
  mbtiInfo.innerHTML = `
    <div style="display:flex; align-items:center; gap:12px; padding:16px; background:#F9F6FF; border-radius:8px;">
      <div style="width:60px; height:60px; background:#7C3AED; border-radius:50%; display:flex; align-items:center; justify-content:center; color:white; font-size:18px; font-weight:bold;">MBTI</div>
      <div>
        <div style="font-size:16px; font-weight:600; color:#333;">${mbtiMatch}</div>
        <div style="font-size:14px; color:#666; margin-top:4px;">SBTI 与 MBTI 类型对应参考</div>
      </div>
    </div>
  `;
...
function autoTest() {
  ...
  app.visibleQuestions = app.shuffledQuestions.filter(q => {
    if (q.id === 'drink_gate_q1') {
      app.answers[q.id] = 2;
      return true;
    }
    if (q.id === 'drink_gate_q2' && app.answers['drink_gate_q1'] === 3) {
      app.answers[q.id] = 1;
      return true;
    }
    if (q.id !== 'drink_gate_q2' || app.answers['drink_gate_q1'] === 3) {
      app.answers[q.id] = Math.floor(Math.random() * 3) + 1;
      return true;
    }
    return false;
  });
...
async function validateCode() {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/validate-code`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ code, testType: 'sbti' })
    });
...
function saveResultImage() {
  ...
  html2canvas(resultLayout, {
    ...
    link.download = `SBTI-人格测试结果-${new Date().getTime()}.png`;
